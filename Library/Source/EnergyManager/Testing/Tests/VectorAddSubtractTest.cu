	...
	namespace Testing {
		namespace Tests {
			...
			VectorAddSubtractTest::VectorAddSubtractTest(
				...
				: Test("Vector Add Subtract", monitors)
				, gpu_(gpu)
				, computeCount_(computeCount) {
			}

			VectorAddSubtractTest::VectorAddSubtractTest(const std::map<std::string, std::string>& arguments)
				: VectorAddSubtractTest(
					Hardware::GPU::getGPU(EnergyManager::Utility::Text::getArgument<unsigned int>(arguments, "--gpu", 0)),
					Utility::Text::getArgument<unsigned int>(arguments, "--computeCount", 50000),
