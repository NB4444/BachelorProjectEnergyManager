...
static void monitor_sleep() {
	...
	debug("going to sleep %llu ms (%ld, %ld)", next_wake_ms, next_wake_ts.tv_sec, next_wake_ts.tv_nsec);
...
static void monitor_time_calc(suscribers_t* s) {
	...
	debug("-- time since %llu, time aux %llu ms", timestamp_convert(&s->time_since, TIME_MSECS), timestamp_convert(&time_aux, TIME_MSECS));
	//
	s->time_passed += timestamp_diff(&s->time_since, &time_aux, TIME_MSECS);
	//
	s->time_follow = s->time_limit - s->time_passed;
	//
	debug("-- time limit %llu, time passed %llu ms, time next %lld", s->time_limit, s->time_passed, s->time_follow);
	//
...
state_t suscriptor_suscribe(void* _s) {
	...
	if(n->api_cntx == NULL)
		return_msg(EAR_BAD_ARGUMENT, "context is NULL");
	if(n->api_call == NULL)
		return_msg(EAR_BAD_ARGUMENT, "reading call is NULL");
	if(n->api_memm == NULL)
		return_msg(EAR_BAD_ARGUMENT, "reading memory is NULL");
