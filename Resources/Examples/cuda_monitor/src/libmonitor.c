...
void CUPTIAPI cuda_callback(void *userdata, CUpti_CallbackDomain domain,
  ...
  if (cbInfo->callbackSite != CUPTI_API_ENTER) {
    ...
    fprintf(stderr, "%s;%lu;%lu;%llu;%llu;%s;%u;%lu;%lu\n",
            cbInfo->functionName, proc, thrd, to, ts, site, cupti_calls[cbid],
            fret, fpar);
  }
...
void __attribute__((constructor)) module_cuda() {
  ...
  tprintf("Function name||Proc||Thread||Timestamp||Time plus||Site||#||Ret "
          "val.||Ret par.");
