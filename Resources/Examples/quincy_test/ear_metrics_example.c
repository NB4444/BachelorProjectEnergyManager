/*    This program is part of the Energy Aware Runtime (EAR).
    It has been developed in the context of the BSC-Lenovo Collaboration
   project.
...
void usage() {
  printf("Usage: ear_metrics_example n_threads n_iterations def_frequency "
         "number_of_runs_per_test energy_plugin_path\n");
  ...
  printf("Use export USE_MONITOR=1 to start the additional monitor thread "
         "required for dcmi energy plugin\n");
  exit(1);
}
...
int test_available(int i, int cpu) { return 1; }
...
int main(int argc, char *argv[]) {
  ...
  printf("%s executing: %d threads , %d iterations at freq %lu. %d runs per "
         "test\n",
  ...
  metrics_init =
      calloc(my_topo.socket_count * RAPL_POWER_EVS, sizeof(unsigned long long));
  metrics_end =
      calloc(my_topo.socket_count * RAPL_POWER_EVS, sizeof(unsigned long long));
  metrics_diff =
      calloc(my_topo.socket_count * RAPL_POWER_EVS, sizeof(unsigned long long));
...
#pragma omp parallel firstprivate(run, test, i_test, n_iterations, cpu)        \
    num_threads(n_threads) if (do_test_in_parallel(i_test))
          {
#pragma omp barrier
...
#endif
#if TEST_DC_POWER
              if (energy_dc_time_read(&my_eh, my_energy_data_init,
                                      &start_time) != EAR_SUCCESS) {
...
#if TEST_DC_POWER
              if (energy_dc_time_read(&my_eh, my_energy_data_end, &end_time) !=
                  EAR_SUCCESS) {
                fprintf(stderr, "Error reading energy\n");
              }
...
/* Reading and computing RAPL energy values */
...
#if TEST_DC_POWER
          /* Node energy */
          energy_accumulated(&my_eh, &dc_energy_j, my_energy_data_init,
                             my_energy_data_end);
          ...
          if (full_compatible) {
#if TEST_FLOPS
            total_fp_inst = total_flops[0] + total_flops[1] + total_flops[2] +
                            total_flops[3] + total_flops[4] + total_flops[5] +
                            total_flops[6] + total_flops[7];
#endif
          }
          energy_raw = 0;
          energy_dram_raw = 0;
          ...
          for (i_socket = 0; i_socket < n_sockets; ++i_socket) {
            ...
            energy_raw +=
                (double)metrics_diff[n_sockets * RAPL_PCK_EV + i_socket];
            energy_dram_raw +=
                (double)metrics_diff[n_sockets * RAPL_DRAM_EV + i_socket];
          }
          // Raw data
          ...
          printf("\t(Gflops_met x n_thread)/dc_power_watt: %.3lf\n",
                 (float)(papi_flops * n_threads) /
                     (float)(power_dc_w * 1000000000));
          printf("\tTotal FP_OPS x n_threads(millions): based on test %lu, "
                 "per_th %lu\n",
          ...
          for (i = 0; i < my_topo.socket_count; i++) {
            printf("\tSocket [%d] Power PCK %.2f DRAM %.2f Temp %llu\n", i,
                   (float)metrics_diff[n_sockets * RAPL_PCK_EV + i] /
                       (float)(time_s * 1000000000),
                   (float)metrics_diff[n_sockets * RAPL_DRAM_EV + i] /
                       (float)(time_s * 1000000000),
                   rapl_temp[i]);
          }
          printf("\tUncore counters %s \n", unc);
          printf("\tFlops[0]=%lld Flops[1]=%lld Flops[2]=%lld Flops[3]=%lld "
                 "Flops[4]=%lld Flops[5]=%lld Flops[6]=%lld Flops[7]=%lld\n",
                 total_flops[0], total_flops[1], total_flops[2], total_flops[3],
                 total_flops[4], total_flops[5], total_flops[6],
                 total_flops[7]);
