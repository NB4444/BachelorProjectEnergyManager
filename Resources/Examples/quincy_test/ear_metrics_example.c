...
void usage() {
	printf(
		"Usage: ear_metrics_example n_threads n_iterations def_frequency "
	...
	printf(
		"Use export USE_MONITOR=1 to start the additional monitor thread "
...
int main(int argc, char* argv[]) {
	...
	printf(
		"%s executing: %d threads , %d iterations at freq %lu. %d runs per "
		"test\n",
		argv[0],
		n_threads,
		d_iterations,
...
/* Reading and computing RAPL energy values */
					...
					if(full_compatible) {
#if TEST_FLOPS
						total_fp_inst = total_flops[0] + total_flops[1] + total_flops[2] + total_flops[3] + total_flops[4] + total_flops[5] + total_flops[6] + total_flops[7];
#endif
					}
					energy_raw = 0;
					energy_dram_raw = 0;
					...
					printf(
						"\tTotal FP_OPS x n_threads(millions): based on test %lu, "
						"per_th %lu\n",
						num_ops / 1000000,
						num_ops / (n_threads * 1000000));
					...
					for(i = 0; i < my_topo.socket_count; i++) {
						printf(
							"\tSocket [%d] Power PCK %.2f DRAM %.2f Temp %llu\n",
							i,
							(float) metrics_diff[n_sockets * RAPL_PCK_EV + i] / (float) (time_s * 1000000000),
							(float) metrics_diff[n_sockets * RAPL_DRAM_EV + i] / (float) (time_s * 1000000000),
							rapl_temp[i]);
					}
					printf("\tUncore counters %s \n", unc);
					printf(
						"\tFlops[0]=%lld Flops[1]=%lld Flops[2]=%lld Flops[3]=%lld "
