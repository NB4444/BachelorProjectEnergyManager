...
class DataSet(object):
    ...
    @cached_property
    def table_plot(self):
        return TablePlot(title="Profiler Sessions", table=[[profiler_session.id, profiler_session.label, profiler_session.profile] for profiler_session in self.data],
                         columns=["ID", "Label", "Profile"])

    @cached_property
    def summary(self):
        return Containers.unique_dict_of_lists(Containers.merge_dictionaries([profiler_session.summary for profiler_session in self.data]))

    @cached_property
    def monitor_data(self):
        return Containers.unique_dict_of_lists(Containers.merge_dictionaries(
            [variables for profiler_session in self.data for monitor_session in profiler_session.monitor_sessions for _, variables in monitor_session.namespaced_monitor_data.items()]))

    def clock_rate_histogram_plots(self, bins=30, title_filter: str = ""):
        return [HistogramPlot(title=title, values=values, bins=bins, x_label="Clock Rate (Hertz)") for title, values in self.monitor_data.items() if
                title.endswith("coreClockRate") and title_filter in title]

    def utilization_rate_histogram_plots(self, bins=30, title_filter: str = ""):
        return [HistogramPlot(title=title, values=values, bins=bins, x_label="Utilization Rate (%)") for title, values in self.monitor_data.items() if
                title.endswith("coreUtilizationRate") and title_filter in title]

    def energy_consumption_histogram_plot(self, bins=30):
        return HistogramPlot(title="Energy Consumption", values=[profiler_session.total_energy_consumption for profiler_session in self.data], bins=bins, x_label="Energy Consumption (Joules)")
    ...
    def runtime_histogram_plot(self, bins=30):
        return HistogramPlot(title="Runtime", values=[Plot.ns_to_s(profiler_session.total_runtime) for profiler_session in self.data], bins=bins, x_label="Runtime (Seconds)")
    ...
    def flops_histogram_plot(self, bins=30):
        return HistogramPlot(title="FLOPs", values=[profiler_session.total_flops for profiler_session in self.data], bins=bins)
    ...
    def runtime_vs_energy_consumption_vs_combined_clock_rates(self, normalized=True):
        ...
        for profiler_session in self.data:
            profile = f"{profiler_session.maximum_cpu_clock_rate_string}\n{profiler_session.maximum_gpu_clock_rate_string}"
            # profile = "Runs"
    ...
    def energy_consumption_vs_runtime_scatter_plot(self, normalized=True):
        ...
        for profiler_session in self.data:
            if "maximumCPUClockRate" in profiler_session.profile:
                values.append(profiler_session.profile["maximumCPUClockRate"] + profiler_session.profile["maximumGPUClockRate"])
        if len(values) > 0:
            max_value = max(values)
            min_value = min(values)
            colors = [pyplot.get_cmap("gist_rainbow")((value - min_value) / (max_value - min_value)) for value in values]
        else:
            colors = None

        colors = None

        return ScatterPlot(title="Runtime vs. Energy Consumption vs. Combined Clock Rates", plot_series=plot_series,
                           x_label="Runtime (" + ("% of optimal" if normalized else "Seconds") + ")",
                           y_label="Energy Consumption (" + ("% of optimal" if normalized else "Joules") + ")", colors=colors)
    ...
    def energy_consumption_vs_flops_scatter_plot(self, normalized=True):
        ...
        return ScatterPlot(title="Energy Consumption vs. FLOPs", plot_series=plot_series,
    ...
    def core_clock_rate_vs_gpu_clock_rate_vs_energy_consumption(self):
        ...
        for profiler_session in self.data:
            # profile = f"{profiler_session.maximum_cpu_clock_rate_string}\n{profiler_session.maximum_gpu_clock_rate_string}"
            # profile = str(profiler_session.profile["maximumCPUClockRate"] + profiler_session.profile["maximumGPUClockRate"])
            profile = "Runs"
            if profile not in data:
                data[profile] = {}
    ...
    def core_clock_rate_vs_gpu_clock_rate_vs_energy_consumption_scatter_plot(self):
        ...
        return ScatterPlot(title="Core Frequency vs. GPU Frequency vs. Energy Consumption", plot_series=self.core_clock_rate_vs_gpu_clock_rate_vs_energy_consumption, x_label="Core Clock Rate (Hertz)",
                           y_label="GPU Clock Rate (Hertz)", z_label="Energy Consumption (Joules)", colors=colors)
    ...
    def core_clock_rate_vs_gpu_clock_rate_vs_runtime_scatter_plot(self):
        ...
        return ScatterPlot(title="Core Frequency vs. GPU Frequency vs. Runtime", plot_series=plot_series, x_label="Core Clock Rate (Hertz)",
                           y_label="GPU Clock Rate (Hertz)", z_label="Runtime (Seconds)", colors=colors)
