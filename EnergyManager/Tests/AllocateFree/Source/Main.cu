#include "Workloads/AllocateFreeWorkload.hpp"
...
int main(int argumentCount, char* argumentValues[]) {
	...
	const auto deviceAllocations = EnergyManager::Utility::Text::getArgument<unsigned int>(arguments, "--deviceAllocations", 3000000);
	const auto deviceSize = EnergyManager::Utility::Text::getArgument<unsigned long>(arguments, "--deviceSize", 1024);
	...
	EnergyManager::Testing::TestRunner testRunner(
		{ std::make_shared<EnergyManager::Testing::Tests::WorkloadTest>(name, std::make_shared<Workloads::AllocateFreeWorkload>(hostAllocations, hostSize, deviceAllocations, deviceSize), monitors) });
