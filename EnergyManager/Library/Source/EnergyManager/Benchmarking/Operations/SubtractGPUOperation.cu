#include "./SubtractGPUOperation.hpp"
...
namespace EnergyManager {
	namespace Benchmarking {
		namespace Operations {
			...
			void SubtractGPUOperation::onRun() {
				...
				subtract<<<blocksPerGrid, threadsPerBlock_>>>(leftVariable, rightVariable, resultVariable);
				ENERGY_MANAGER_HARDWARE_GPU_HANDLE_API_CALL(cudaGetLastError());
			}

			SubtractGPUOperation::SubtractGPUOperation(const unsigned int& count, const unsigned int& threadsPerBlock) : count_(count), threadsPerBlock_(threadsPerBlock) {
			}
		}
	}
}