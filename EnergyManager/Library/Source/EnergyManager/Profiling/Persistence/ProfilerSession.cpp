	...
	namespace Profiling {
		namespace Persistence {
			void ProfilerSession::onSave() {
				std::map<std::string, std::string> columnValues = { { "label", '\'' + filterSQL(getLabel()) + '\'' } };
				if(getID() >= 0) {
					columnValues["id"] = Utility::Text::toString(getID());
				}
				setID(insert("ProfilerSession", columnValues));
			...
			ProfilerSession::ProfilerSession(std::string label, std::map<std::string, std::string> profile, std::vector<std::shared_ptr<Monitoring::Persistence::MonitorSession>> monitorSessions)
			...
			std::vector<std::shared_ptr<Monitoring::Persistence::MonitorSession>> ProfilerSession::getMonitorSessions() const {
				return monitorSessions_;
			}

			void ProfilerSession::setMonitorSessions(const std::vector<std::shared_ptr<Monitoring::Persistence::MonitorSession>>& monitorSessions) {
