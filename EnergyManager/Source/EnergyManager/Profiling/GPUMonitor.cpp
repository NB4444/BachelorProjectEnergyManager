...
namespace EnergyManager {
	namespace Profiling {
		std::map<std::string, std::string> GPUMonitor::onPoll() {
			auto gpuResults = std::map<std::string, std::string> { { "computeCapabilityMajorVersion", std::to_string(gpu_.getComputeCapabilityMajorVersion()) },
		...
		GPUMonitor::GPUMonitor(const Hardware::GPU& gpu) : ProcessorMonitor("GPUMonitor", gpu), gpu_(gpu) {
		}
	}
}