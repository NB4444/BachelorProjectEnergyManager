...
namespace EnergyManager {
	namespace Utility {
		namespace Units {
			...
			public:
				PerUnit(const Type& unit, const Per& perUnit, const std::string& perUnitName = "", const std::string& perUnitSymbol = "")
					: unit_(unit)
					, perUnit_(perUnit)
					, perUnitName_(perUnitName)
					, perUnitSymbol_(perUnitSymbol) {
				}
				...
				std::string toString(const bool& symbol = false) const {
					return std::to_string(unit_) + " " + (symbol ? perUnitSymbol_ : (perUnitName_ == "" ? "per " + std::to_string(perUnit_) : perUnitName_));
				}
