...
static void printActivity(CUpti_Activity* record) {
	switch(record->kind) {
		...
		case CUPTI_ACTIVITY_KIND_MEMSET: {
			...
			printf(
				"MEMSET value=%u [ %llu - %llu ] device %u, context %u, stream %u, correlation %u\n",
				memset->value,
		...
		case CUPTI_ACTIVITY_KIND_CONCURRENT_KERNEL: {
			...
			printf(
				"%s \"%s\" [ %llu - %llu ] device %u, context %u, stream %u, correlation %u\n",
				kindString,
				kernel->name,
		...
		case CUPTI_ACTIVITY_KIND_MARKER_DATA: {
			...
			printf(
				"MARKER_DATA id %u, color 0x%x, category %u, payload %llu/%f\n",
				marker->id,
				marker->color,
				marker->category,
